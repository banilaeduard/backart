...
namespace YahooTFeeder
{
    ...
    internal sealed class YahooTFeeder : StatelessService, IYahooFeeder
    {
        ...
        public async Task ReadMails(string sourceName)
        {
            await ReadMails(sourceName, CancellationToken.None);
        }

        async Task ReadMails(string sourceName, CancellationToken cancellationToken)
        {
        ...
        public async Task<MailBody[]> DownloadAll(TableEntityPK[] uids, string sourceName)
        {
            var mailSettings = new MailSettingsRepository(null);
            var ticketEntryRepository = new TicketEntryRepository(null);
            var blob = new BlobAccessStorageService();
